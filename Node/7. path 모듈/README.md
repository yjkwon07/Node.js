...
__경로확인__
```javascript
console.log(path.dirname(__filename)); // 현재 파일 경로
console.log(path.extname(__filename)); // 확장자 이름
console.log(path.basename(__filename)); // 파일명
console.log(path.parse(__filename)); // 구성요소 쪼개기(객체 불러옴)
...
console.log(path.resolve(__dirname,'..', '..', '\\Node')); // 정대 경로 고려하고 합침 루트는 C:\
// out : c:\Node
```